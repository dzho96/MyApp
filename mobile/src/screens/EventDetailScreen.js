...
function normalizeDraftRecurrence(recurrence) {
  if (!recurrence) return null

  return {
    frequency: recurrence.frequency || recurrence.type || 'weekly',
    interval: String(recurrence.interval || 1),
    endMode: recurrence.count ? 'count' : recurrence.until ? 'until' : 'never',
    untilDate: recurrence.until ? toDateOrNull(`${recurrence.until}T00:00:00`) : null,
    count: recurrence.count ? String(recurrence.count) : '10'
  }
}

function buildDraftRecurrenceRule(draftRecurrence) {
  ...
  const frequency = draftRecurrence.frequency
  ...
  if (draftRecurrence.endMode === 'until') {
    if (!draftRecurrence.untilDate) {
      throw new Error('Pick an end date or choose a different repeat end condition.')
    }
    until = draftRecurrence.untilDate.toISOString().slice(0, 10)
  }

  if (draftRecurrence.endMode === 'count') {
    count = parseInt(draftRecurrence.count, 10)
    if (!count || count < 1) {
...
export default function EventDetailScreen({ route, navigation }) {
  ...
  const [draftReminders, setDraftReminders] = useState(() => draft?.reminders || [])
  const [draftRecurrence, setDraftRecurrence] = useState(() => normalizeDraftRecurrence(draft?.recurrence))
  ...
  function addDraftTask() {
    ...
    setDraftTasks((previous) => [...previous, trimmed])
    setNewDraftTaskName('')
  }

  function removeDraftTask(index) {
    setDraftTasks((previous) => previous.filter((_, taskIndex) => taskIndex !== index))
  }
  ...
  function resolveDraftReminders() {
    ...
    for (let index = 0; index < draftReminders.length; index += 1) {
      ...
      if (reminder.kind === 'absolute') {
        remindAt = toDateOrNull(reminder.remindAt)
        if (!remindAt) {
        ...
        const anchorDate = anchor === 'end' ? endTime : startTime
  ...
  function validateDraft() {
    ...
    return {
      reminderTimes: isEdit ? [] : resolveDraftReminders(),
      recurrenceRule: isEdit ? null : buildDraftRecurrenceRule(draftRecurrence)
    }
  }
  ...
  function handleSave() {
    ...
    try {
      ...
      Alert.alert('Cannot save event', err.message || 'Please review the event details.')
      return
    }

    const startedMoreThanOneHourAgo =
      !isEdit &&
      startTime.getTime() < Date.now() - 60 * 60 * 1000

    if (startedMoreThanOneHourAgo) {
      Alert.alert(
        ...
        [
          ...
          { text: 'Create anyway', onPress: () => doSave(validated) }
        ]
      )
      return
    }
  ...
  async function doSave({ reminderTimes, recurrenceRule }) {
    const payload = {
      name: name.trim(),
    ...
    try {
      ...
      if (!newEventId) {
        throw new Error('No id returned for new event.')
      }
      ...
      for (const remindAt of reminderTimes) {
        ...
        if (!result?.id) {
          throw new Error('A reminder was created but no id was returned.')
        }
        createdReminders.push(result)
      }

      if (recurrenceRule) {
        await setRecurrence(newEventId, recurrenceRule)
      }
      ...
      for (const reminder of createdReminders) {
        try {
          await scheduleReminderNotification(
            {
              ...reminder,
              event_id: reminder.event_id || newEventId,
          ...
          console.warn('Could not schedule local reminder notification:', notificationErr)
        }
      }
    ...
    <ScrollView style={{ backgroundColor: colors.surfaceMuted }} contentContainerStyle={styles.container}>
      {showParseNotes && parseNotes.length > 0 && (
        <View style={[styles.parseBanner, { backgroundColor: colors.surfaceMuted, borderColor: colors.borderDefault }]}>
          <Text style={{ color: colors.textSecondary, fontSize: 12, marginBottom: 4 }}>
            Pre-filled from Quick Add — review and edit before saving.
          </Text>
      ...
      <View style={[styles.input, { borderColor: colors.borderDefault, backgroundColor: colors.surface, padding: 0 }]}>
        <Picker selectedValue={category} onValueChange={setCategory} style={{ color: colors.textPrimary }}>
          ...
          {CATEGORIES.map((item) => <Picker.Item key={item} label={item} value={item} />)}
        </Picker>
      </View>
      ...
      {!isEdit && (
        <>
          <FormSection title="Repeats" subtitle="Make this a recurring event" defaultOpen={!!draftRecurrence}>
          ...
          <FormSection
            title="Reminders"
            subtitle={draftReminders.length > 0 ? `${draftReminders.length} added` : 'Get notified before this event'}
            defaultOpen={draftReminders.length > 0}
          >
      ...
      <FormSection
        ...
        subtitle={
          isEdit
            ...
            : (draftTasks.length > 0 ? `${draftTasks.length} added` : null)
        }
      >
        {isEdit ? (
          tasks.map((task) => (
...
function DraftRecurrenceSection({ value, onChange, colors }) {
  const enabled = !!value
  ...
  function openUntilPicker() {
    if (Platform.OS === 'android') {
      DateTimePickerAndroid.open({
        value: value?.untilDate || new Date(),
        mode: 'date',
        minimumDate: new Date(),
        onChange: (event, date) => {
          if (event.type === 'set' && date) updateRule({ untilDate: date })
        }
      })
    } else {
        ...
        <>
          <View style={styles.customOffsetRow}>
            ...
            <TextInput
              ...
              onChangeText={(interval) => updateRule({ interval })}
            ...
            <View style={[styles.offsetUnitPicker, { borderColor: colors.borderDefault, backgroundColor: colors.surface }]}>
              <Picker
                selectedValue={value.frequency}
                onValueChange={(frequency) => updateRule({ frequency })}
          ...
          <View style={styles.anchorRow}>
            {[
              ...
              <TouchableOpacity
                key={option.value}
                onPress={() => updateRule({ endMode: option.value })}
                style={[
                  styles.anchorChip,
                  {
                    borderColor: colors.borderDefault,
                    backgroundColor: value.endMode === option.value ? colors.accentPrimary : colors.surface
                  }
                ]}
              >
                <Text style={{ color: value.endMode === option.value ? colors.surface : colors.textPrimary, fontWeight: '600', fontSize: 12 }}>
                  {option.label}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            <>
              ...
              >
                <Text style={{ color: colors.textPrimary }}>
                  {value.untilDate ? `Ends: ${value.untilDate.toLocaleDateString()}` : 'Pick end date'}
                </Text>
              </TouchableOpacity>
          ...
          {value.endMode === 'count' && (
            <View style={styles.customOffsetRow}>
              ...
              <TextInput
                value={value.count}
                onChangeText={(count) => updateRule({ count })}
                keyboardType="numeric"
                style={[styles.offsetAmountInput, { borderColor: colors.borderDefault, color: colors.textPrimary, backgroundColor: colors.surface }]}
              />
...
function DraftReminderSection({ startTime, endTime, reminders, onChange, colors }) {
  ...
  function handlePresetPress(preset) {
    addRelativeReminder(preset.amount, preset.unit)
  }

  function handleCustomOffsetSubmit() {
    addRelativeReminder(parseFloat(offsetAmount), OFFSET_UNITS[offsetUnitIndex].label)
  }

  function addAbsoluteReminder(date) {
    if (!date || Number.isNaN(date.getTime())) {
      Alert.alert('Invalid time', 'Pick a valid reminder date and time')
      return
    }

    onChange([
      ...reminders,
      { kind: 'absolute', remindAt: date.toISOString() }
    ])
  }
  ...
  function describeReminder(reminder) {
    if (reminder.kind === 'absolute') {
      const date = toDateOrNull(reminder.remindAt)
      return date ? date.toLocaleString() : 'Custom reminder (invalid date)'
    }

    return `${reminder.amount} ${reminder.unit} before ${reminder.anchor === 'end' ? 'end' : 'start'}`
  }
  ...
  return (
    <View>
      ...
      {reminders.map((reminder, index) => (
        <View key={`${reminder.kind}-${index}`} style={styles.taskRow}>
          <Text style={{ flex: 1, color: colors.textPrimary }}>
            {describeReminder(reminder)}
          </Text>
          <TouchableOpacity onPress={() => onChange(reminders.filter((_, itemIndex) => itemIndex !== index))}>
            <Text style={{ color: colors.textMuted, fontSize: 18 }}>×</Text>
          </TouchableOpacity>
        </View>
      ))}
      ...
      <View style={styles.tabRow}>
        ...
        >
          <Text style={{ color: tab === 'quick' ? colors.surface : colors.textPrimary, fontWeight: '700', fontSize: 13 }}>
            Quick
          </Text>
        </TouchableOpacity>
        ...
        >
          <Text style={{ color: tab === 'custom' ? colors.surface : colors.textPrimary, fontWeight: '700', fontSize: 13 }}>
            Custom
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {tab === 'quick' && (
        <>
          ...
          <View style={styles.offsetRow}>
            {REMINDER_OFFSETS.map((preset) => (
              <TouchableOpacity
                key={preset.label}
                onPress={() => handlePresetPress(preset)}
                style={[styles.offsetChip, { borderColor: colors.borderDefault, backgroundColor: colors.surface }]}
              >
                <Text style={{ color: colors.textPrimary, fontSize: 12 }}>{preset.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <TouchableOpacity
            ...
            style={[styles.sectionSubmitBtn, { backgroundColor: colors.accentPrimary }]}
          >
      ...
      {tab === 'custom' && (
        <>
          <TouchableOpacity
            ...
            style={[styles.sectionSubmitBtn, { backgroundColor: colors.accentPrimary, alignSelf: 'flex-start', paddingHorizontal: 16 }]}
          >
            <Text style={{ color: colors.surface, fontWeight: '700' }}>
              Pick date & time
            </Text>
          ...
          {Platform.OS === 'ios' && showCustomPicker && (
            <DateTimePicker
              ...
              onChange={(event, date) => {
                ...
                if (event.type === 'set' && date) addAbsoluteReminder(date)
              }}
            />
          )}
        </>
  ...
  useEffect(() => {
    fetchRecurrence(eventId)
      .then((rule) => {
        ...
        if (rule.until) {
          setEndMode('until')
          setUntilDate(new Date(`${rule.until}T00:00:00`))
        } else if (rule.count) {
          setEndMode('count')
        ...
        <>
          <View style={styles.customOffsetRow}>
            ...
            <View style={[styles.offsetUnitPicker, { borderColor: colors.borderDefault, backgroundColor: colors.surface }]}>
              <Picker selectedValue={frequency} onValueChange={setFrequency} style={{ color: colors.textPrimary }}>
                {RECURRENCE_UNITS.map((unit) => <Picker.Item key={unit.value} label={unit.label} value={unit.value} />)}
              </Picker>
            </View>
          </View>
          ...
          <View style={styles.anchorRow}>
            {[
              ...
              <TouchableOpacity
                key={option.value}
                onPress={() => setEndMode(option.value)}
                style={[
                  styles.anchorChip,
                  {
                    borderColor: colors.borderDefault,
                    backgroundColor: endMode === option.value ? colors.accentPrimary : colors.surface
                  }
                ]}
              >
                <Text style={{ color: endMode === option.value ? colors.surface : colors.textPrimary, fontWeight: '600', fontSize: 12 }}>
                  {option.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          {endMode === 'until' && (
            <>
              <TouchableOpacity onPress={openUntilPicker} style={[styles.input, { borderColor: colors.borderDefault, backgroundColor: colors.surface }]}>
                <Text style={{ color: colors.textPrimary }}>
...
function ReminderSection({ eventId, startTime, endTime, colors }) {
  ...
  async function loadReminders() {
    try {
      ...
      const active = list.filter((reminder) => !reminder.dismissed)
      setReminders(active)
  ...
  function handleOffsetPress(minutes) {
    handleAddReminder(new Date(anchorTime.getTime() - minutes * 60 * 1000))
  }
  ...
  function handleCustomOffsetSubmit() {
    ...
    const totalMinutes = amount * OFFSET_UNITS[offsetUnitIndex].minutesPerUnit
    handleAddReminder(new Date(anchorTime.getTime() - totalMinutes * 60 * 1000))
  }
            ...
            <>
              ...
              <View style={styles.offsetRow}>
                {REMINDER_OFFSETS.map((option) => (
                  <TouchableOpacity
                    key={option.label}
                    onPress={() => handleOffsetPress(option.amount * OFFSET_UNITS.find((unit) => unit.label === option.unit).minutesPerUnit)}
                    disabled={saving}
                    style={[styles.offsetChip, { borderColor: colors.borderDefault, backgroundColor: colors.surface, opacity: saving ? 0.6 : 1 }]}
                  >
                    <Text style={{ color: colors.textPrimary, fontSize: 12 }}>{option.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
