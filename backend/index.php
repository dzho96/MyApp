...
try {
    ...
    if (strpos($path, '/api/events') === 0) {
        ...
        if ($method === 'GET') {
            try {
                $pdo = get_pdo();
                $stmt = $pdo->query('SELECT id, name, description, start_time, end_time, category FROM events ORDER BY start_time NULLS LAST LIMIT 100');
        ...
        if ($method === 'POST') {
            ...
            // Basic validation: name length and datetime formats
            $errors = [];
            ...
            try {
                $pdo = get_pdo();
                $sql = 'INSERT INTO events (name, description, start_time, end_time, category, game_id, user_id, source, source_url, is_automatic, is_confirmed) VALUES (:name, :description, :start_time, :end_time, :category, :game_id, :user_id, :source, :source_url, :is_automatic, :is_confirmed) RETURNING id';
        ...
        if (preg_match('#^/api/events/(\d+)$#', $path, $matches)) {
            ...
            if ($method === 'PUT') {
                ...
                try {
                    $pdo = get_pdo();
                    $sql = 'UPDATE events SET name = :name, description = :description, start_time = :start_time, end_time = :end_time, category = :category, updated_at = now() WHERE id = :id';
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute([
                        ':name' => $body['name'],
                        ':description' => $body['description'] ?? null,
